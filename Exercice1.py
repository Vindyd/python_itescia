...
def anneeMoisJour():  # Fonction pour obtenir le jour de la date entrée par un utilisateur
    ...
    while (annee < int(1947)) or (annee == int(1582) and mois < int(11)) or (mois > 12) or (jour > 31) or (mois < 0) or (jour < 0):
        print("")
    ...
    # Si c'est une annee Bissextile, on retire 1 à la somme
    if (annee % 400 == 0) or (annee % 4 == 0 and annee % 100 != 0):
        somme = somme - 1

    # Si c'est le mois de Janvier, on retire 1 à la somme
    if mois == 1:
        somme = somme - 1

    # Si c'est le mois  de Février, on retire 1 à la somme
    if mois == 2:
        somme = somme - 1
